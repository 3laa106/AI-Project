    ...
    def get_possible_moves(self, game):
        ...
        for i in range(game.size):
            for j in range(game.size):
                if game.board[i][j] == 0:
                    # Check if there are stones in adjacent cells
                    has_neighbor = False
                    for di in [-1, 0, 1]:
                        for dj in [-1, 0, 1]:
                            if di == 0 and dj == 0:
                        ...
                        if has_neighbor:
                            break
                    if has_neighbor or np.all(game.board == 0):  # Include all if board is empty
                        empty_cells.append((i, j))
        return empty_cells if empty_cells else [(game.size//2, game.size//2)]  # Center if board is empty
